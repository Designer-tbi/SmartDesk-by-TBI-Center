...
companyRouter.put('/', async (req, res, next) => {
  try {
    const { name, taxId, rccm, idNat, email, phone, website, address, country, state, logo } = req.body;
    await req.db.query('UPDATE companies SET name = $1, "taxId" = $2, rccm = $3, "idNat" = $4, email = $5, phone = $6, website = $7, address = $8, country = $9, state = $10, logo = $11 WHERE id = $12',
      [name, taxId, rccm, idNat, email, phone, website, address, country || 'FR', state, logo || null, req.user!.companyId]);
