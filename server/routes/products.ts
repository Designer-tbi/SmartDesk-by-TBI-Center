...
productsRouter.post('/', async (req, res, next) => {
  try {
    ...
    await logActivity(req.user!.id, req.user!.companyId, 'CREATE_PRODUCT', `Nouveau produit ajouté: ${name} (SKU: ${sku})`);
...
productsRouter.put('/:id', async (req, res, next) => {
  try {
    ...
    await logActivity(req.user!.id, req.user!.companyId, 'UPDATE_PRODUCT', `Produit mis à jour: ${name}`);
...
productsRouter.delete('/:id', async (req, res, next) => {
  try {
    ...
    await logActivity(req.user!.id, req.user!.companyId, 'DELETE_PRODUCT', `Produit supprimé (ID: ${id})`);
