            ...
            <div key={role.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-all group">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex gap-1 sm:opacity-0 sm:group-hover:opacity-100 transition-all sm:translate-x-2 sm:group-hover:translate-x-0">
                  ...
                  <button onClick={() => { setDeleteConfirmId(role.id); setDeleteConfirmType('role'); }} className="p-2 text-slate-400 hover:text-red-600 hover:bg-white rounded-xl transition-all shadow-sm" title={t('common.delete')}>
                    <Trash2 className="w-4 h-4" />
                  </button>
                </div>
              </div>
              ...
              <p className="text-sm text-slate-500 mb-4">
                {role.permissions.includes('all') 
                  ? t('users.fullAccess') 
                  : t('users.permissionsGranted', { count: role.permissions.length.toString() })}
              </p>
          ...
          <form onSubmit={handleAddUser} className="bg-white w-full max-w-md rounded-3xl shadow-2xl p-6">
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <input type="text" required value={newUser.name || ''} onChange={e => setNewUser({...newUser, name: e.target.value})} className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm" placeholder={t('users.placeholder.fullName')} />
              </div>
              <div className="space-y-1.5">
